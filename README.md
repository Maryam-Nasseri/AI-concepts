...
**Large Language Models** are generative AI models that, unlike LLMs which segment the input into tokens and predict the next token, convert the text input to sequences of 'concepts' in an embedding space, predict the next 'concept' and then convert the response to natural language.
